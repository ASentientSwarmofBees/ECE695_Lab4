...
int FileRename(char *oldname, char *newname) {
    ...
    for (i = 0; i < FILE_MAX_OPEN_FILES; i++) {
        if (dstrncmp(files[i].fileName, newname, FILE_MAX_FILENAME_LENGTH) == 0) {
            printf("FileRename: ERROR. File already exists with name '%s' at handle %d.\n", newname, i);
            return FILE_FAIL;
        }
    }
    ...
    if (handle == -1) {
        printf("FileRename: ERROR. File matching oldname not found.\n");
        return FILE_FAIL;
    }
    //perform rename
    dstrncpy(files[handle].fileName, newname, FILE_MAX_FILENAME_LENGTH);
    printf("FileRename: Renamed '%s' to '%s'.\n", oldname, newname);
    return FILE_SUCCESS;
}
